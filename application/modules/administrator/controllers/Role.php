...
class Role extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
        ...
        $this->data['list'] = TRUE;
        $this->layout->title(get_phrase('user_role'). ' | ' . SMS);
    ...
    * ********************************************************** */
    public function add() {
        ...
        $this->data['roles'] = $this->role->get_list('roles', array('status' => 1), '','', '', 'id', 'ASC');
        $this->data['add'] = TRUE;
        $this->layout->title(get_phrase('add'). ' ' . get_phrase('role'). ' | ' . SMS);
        $this->layout->view('role/index', $this->data);
    }
    ...
    * ********************************************************** */
    public function edit($id = null) { 
        ...
        $this->data['roles'] = $this->role->get_list('roles', array('status' => 1), '','', '', 'id', 'ASC');
        $this->data['edit'] = TRUE;       
        $this->layout->title(get_phrase('edit'). ' ' . get_phrase('role'). ' | ' . SMS);
        $this->layout->view('role/index', $this->data);
    }
    ...
    * ********************************************************** */
    private function _prepare_role_validation() {
        ...
        $this->form_validation->set_rules('name', get_phrase('role').' '.get_phrase('name'), 'trim|required|callback_name');
        $this->form_validation->set_rules('note', get_phrase('note'), 'trim');
    }
    ...
    * ********************************************************** */ 
    public function name() {
        if ($this->input->post('id') == '') {
            ...
            if ($role) {
                $this->form_validation->set_message('name', get_phrase('already_exist'));
                return FALSE;
            } else {
                return TRUE;
            }
