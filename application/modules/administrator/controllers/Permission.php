...
class Permission extends MY_Controller {
    ...
    * ********************************************************** */
    public function index(){
         ...
         {            
            if(count($this->input->post('operation')) > 0)
            {
               ...
               foreach($this->input->post('operation') AS $key => $value)
               {
                    ...
                    $data['is_status']         = isset($_POST['is_status'][$key]) && !empty($_POST['is_status'][$key]) ? $_POST['is_status'][$key] : 0;
                    $data['is_export']         = isset($_POST['is_export'][$key]) && !empty($_POST['is_export'][$key]) ? $_POST['is_export'][$key] : 0;
                    $data['status']       = 1;
        ...
        $this->layout->title(get_phrase('role_permission'). ' | ' . SMS);
