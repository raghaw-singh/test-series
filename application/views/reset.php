...
<html lang="en">
    <head>
        ...
        <title><?php echo get_phrase('reset_password'). ' | ' . SMS;  ?></title>
                    ...
                    <div class="row">
                        <div class="col-sm-4 col-sm-offset-4 form-box">
                            <div class="form-bottom">
                                <h3 class="font-white bolds"><?= get_phrase('reset_password');?></h3>
                                <form method="post" action="<?= base_url('auth/resetpass');?>" id="reset-pass" autocomplete="off" onsubmit="return false">
                                    <input type="hidden" name="email" value="<?php echo $user->email; ?>" />
                                    <input type="hidden" name="id" value="<?php echo $user->id; ?>" />
                                    <input type="hidden" name="key" value="<?php echo $key; ?>" />
                                    <div class="form-group">
                                        <input type="password" name="password" placeholder="<?= get_phrase('password');?>" value="<?= set_value('password');?>" class="form-username form-control" id="password" />
                                        <span class="text-danger"><?php echo form_error('password'); ?></span>
                                    </div>
                                    <div class="form-group">
                                        <input type="password" name="conf_password" placeholder="<?= get_phrase('confirm').' '. get_phrase('password');?>" value="<?= set_value('password');?>" class="form-username form-control" id="conf_password" />
                                        <span class="text-danger"><?php echo form_error('conf_password'); ?></span>
                                    </div>
                                    <button type="submit" class="btn" id="reset_password"><?= get_phrase('submit');?></button>
                                </form>
